...
//		System.out.println(map);
	...
	public void create_lvl_order(int[] lvl) {
		int idx = 0;
		Node n = new Node(lvl[idx]);
		idx++;
		root = n;
...
//			Right
	...
	public void create_pre_order(int[] pre) {
		idx=0;
		root = create_pre_order_rec(pre);
	}
	int idx=0;
	private Node create_pre_order_rec(int[] pre) {
		if(pre[idx]==-1) {
			idx++;
			return null;
		}
...
//		System.out.println(n.data);
		idx++;
		n.left = create_pre_order_rec(pre);
		n.right =  create_pre_order_rec(pre);
		return n;
	}
}
