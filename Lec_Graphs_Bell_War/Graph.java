...
//			IN total with Loop 1 , O(2E*log(V)) = O(Elog(V))
	...
	public void BellmenFord(int src) {
		ArrayList<EdgePair> Edges = getAllEdges();
...
//			}
	...
	public void FloydWarshall() {
		int[][] dist = new int[map.size() + 1][map.size() + 1];
...
//		Arrays.fill(dist, 1000000);
		...
		for (int V1 : map.keySet()) {
			for (int nbr : map.get(V1).keySet()) {
				dist[V1][nbr] = map.get(V1).get(nbr);
			}
		}

		for (int mid = 1; mid <= map.size(); mid++) {
			for (int i = 1; i <= map.size(); i++) {
				for (int j = 1; j <= map.size(); j++) {
